...
#------------------- configuración ----------------
datadelta=data2                             #  (100: 3, 1000: 2 , 5000: 1, 10000: 0)
intervalo1=[25,1000]  # Intervalo ajuste lineal (100: [25,100], 1000: [25,1000] , 5000: [30,4500], 10000: [30,9500])
lim_escala=100000

dataretorno = data2r
lim_grafico =  [-5200,5200]                #  (100: [-1500,1500], 1000:  [-5200,5200], 5000: [-16000,16000], 10000: [-19000,19000]) 
lim_graficoZOOM=[-100,100]
tau = 1.44                                  #  (100: 1.51, 1000: 1.44, 5000: 1.44, 10000: 1.46)  
initial_guess = [27,0.000005]    #aq, a1
int_graficado =[-4000,4000]    #hasta donde grafico curva ajustada #  (100: [-510,510], 1000:  [-4000,4000], 5000: [-12000,12000], 10000: [-20000,20000])                    
int_ajuste = [-3800,3800]      #en este int hago el ajuste para que el zoom se vea bien (100:[-370,370], 1000:  [-3800,3800], 5000: [-9950,9950], 10000: [-9950,9950] )
int_excluido = [0,0]
ig = False
...
save_fig = False
nombre_archivo = 'AJUSTES_09_norm-val-max_1000'
...
def FUNC_log(x,aq,a1): # Cambiar valores de 1/(1-q) y de (q-1) según cada q
    ...
    return -0.719 * np.log( 1 - (aq/a1) + (aq/a1) * np.exp(1.39*a1*(x**2)) )
...
def grafico(datadelta,lim_escala,intervalo1,   dataretorno,lim_grafico,tau,int_ajuste,int_graficado,initial_guess,ig,int_excluido,  color,marker,label,pwdgraf,nombre_archivo,save_fig):
    ...
    dataretorno_pares = separar_par_impar(dataretorno)[0]  # El ajuste lo hago en la rama par  
    binsretorno = np.linspace(int_ajuste[0]-0.5,int_ajuste[1]+0.5,-int_ajuste[0]+int_ajuste[1]+2)
    y, bin_edges_tot = np.histogram(dataretorno_pares, binsretorno)
    ...
    #ax2.plot(x_proy, 10**(FUNC_log(x_proy, *popt)), color='black', linestyle='--' , linewidth=0.8)#label='Proyección'
    #-------------------------------- 
    xx=1   # Punto para la leyenda
    yy=1
    ax2.scatter(xx, yy, color='none', label=f'$a_q={popt[0]:.3f}$\n$a_1={round(popt[1],11)}$')
    
    if ig == True:
        ax2.plot(x_tot, 10**(FUNC_log(x_tot, initial_guess[0], initial_guess[1])), color='red', linewidth=0.8, linestyle='--',
             label='initial guess')
...
popt = grafico(datadelta,lim_escala,intervalo1,   dataretorno,lim_grafico,tau,int_ajuste,int_graficado,initial_guess,ig,int_excluido,   color,marker,label,pwdgraf,nombre_archivo,save_fig)


def grafico_zoomzoom(dataretorno,lim_graficoZOOM,popt,   color,marker,label,pwdgraf,nombre_archivo,save_fig):
    ...
    # Grafico curva con los parámetros ajustados
              ...
              #[aq,a1]         100: [0.45278174640965785,2.7769049296420798e-05],  1000:  [-3800,3800], 5000: [], 10000: []
    #popt = [0.45278174640965785,2.7769049296420798e-05]
...
grafico_zoomzoom(dataretorno,lim_graficoZOOM,popt,  color,marker,label,pwdgraf,nombre_archivo,save_fig)
